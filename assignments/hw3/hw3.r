...
wr.name <- wr1500m[which(wr1500m$times == min(wr1500m$times)),"athlete"]
...
# times are continous and year is discrete (doesn't have a decimal)
...
qplot(wr1500m$year, wr1500m$times_sec, geom = "line")
...
# Your qplot command:
qp1 <- qplot(wr1500m$new_year, wr1500m$times_sec, geom = "line")
qp1
...
# Your ggplot command:
qp2 <- qp1 + geom_segment(aes(x=1998 ,y=wr_1998 , xend =2015 , yend=wr_1998))
...
# Your ggplot command
qp3 <- qp2 +  geom_vline(xintercept=wr_1944, colour="green")
qp4 <- qp3 + geom_vline(xintercept=wr1500m$new_year[wr1500m$year==1998], colour="green")
qp5 <- qp4 + annotate("text",x=1944+10, y=wr1500m$times_sec[wr1500m$year==1944]+4, label = wr1500m$athlete[wr1500m$year==1944], colour = "blue")
qp6 <- qp5 + annotate("text", x=1998+10, y=wr1500m$times_sec[wr1500m$year==1998]+4, label = wr1500m$athlete[wr1500m$year==1998], colour = "blue")
qp6
...
qp7 <- qp6 + labs(title= "Men's 1500m WR Times", x = "Date", y= "Time in Seconds")
qp7
################################
# PLOT 2
...
qplot(SO2012Ctry$GDP, SO2012Ctry$pop)
...
# Your ggplot command
Medal.Scatter <- qplot(log(SO2012Ctry$GDP_per_person), log(SO2012Ctry$pop)) + 
  geom_point(aes(size = log(SO2012Ctry$Total)/35))
...
Medal.Scatter2<- Medal.Scatter + labs(title= "Medal Count based on Population and GDP Per Person", x = "log(GDP Per Person)", y= "log(Country Population)")
Medal.Scatter2
big5 <- order(SO2012Ctry$Total, decreasing = TRUE )[1:5]

Medal.Scatter3<- Medal.Scatter2 + 
  annotate("text", x=log(SO2012Ctry$GDP_per_person)[big5], y=log(SO2012Ctry$pop)[big5],
           label = SO2012Ctry$Country[big5], colour = "red")
Medal.Scatter3
# Your ggplot command:
...
#start <- qplot(long, lat, data = choro, group = group, geom = "polygon")
start <- ggplot()
graph <- start + geom_polygon(data=world.map , aes(x=long, y=lat, group=group), fill="grey40")
graph
#  mapping = NULL, data=world.map, stat ="identity", position="identity")
...
wonMetal <- SO2012Ctry[SO2012Ctry$Total >0,]
# Your ggplot commands here.
graph2 <- graph + geom_point(aes(x=wonMetal$long, y=wonMetal$lat), size= log(wonMetal$Total),colour = "gold")
graph2
## Not needed
...
plot.gender <-ggplot(data=athletes, aes(Sport, fill = Sex))+geom_bar(position="dodge")
plot.gender
## Skip this question...
...
# Your ggplot commands
plot.gender2 <- plot.gender +theme(axis.text.x = element_text(angle = 90)) + labs(title= "Olympic Sport by Sex", x = "Sport", y= "Count")
plot.gender2

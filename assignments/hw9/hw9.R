...
optionlist <- options()
bub <- gvisBubbleChart(mtcars, idvar = "model", xvar = "disp", yvar = "mpg", colorvar ="gear", sizevar = "hp")
plot(bub)
...
load("/Users/EJGabrielli/Desktop/stat133/assignments/WorldBank.RData")
...
WorldDat <- WorldBank[country, year,fertility.rate, life.expectancy,population,region]

WorldDat <- WorldBank[2:6]
WorldDat$region <- WorldBank$region
...
# for now just use the aboveinstructions as default.

Motion <- gvisMotionChart(WorldDat, idvar="country",
   xvar="life.expectancy", yvar="fertility.rate", 
   sizevar="population", colorvar="region", timevar="year")
